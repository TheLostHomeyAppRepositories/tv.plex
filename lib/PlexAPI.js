  ...
  async requireIP() {
    ...
    if (!this.url) {
      this.url = Promise.resolve().then(async () => {
        ...
        // Try Local
        try {
          const localConnection = server.connections.find(connection => connection.local === '1');
          if (!localConnection) throw new Error('Plex Not Available (Local)');
          const localURL = `${localConnection.protocol}://${localConnection.address}:${localConnection.port}`;

          this.homey.log('[PlexAPI]', `Testing Local URL (${localURL})...`);
          ...
          const cloudConnection = server.connections.find(connection => connection.local === '0');
          if (!cloudConnection) throw new Error('Plex Not Available (Cloud)');
          const cloudURL = `${cloudConnection.protocol}://${cloudConnection.address}:${cloudConnection.port}`;

          this.homey.log('[PlexAPI]', `Testing Cloud URL (${cloudURL})...`);
